...
Load the needed packages. Note: dsbox is not yet on CRAN. For now, you need to install it before you load the library. Uncomment, by deleting # from, the two lin es of code below.
...
1.  What are the dimensions of the Denny's dataset?
    ...
    raw= 1643 , colum= 6
the raw it's about the resturant address (more information about it's)

colum(address , city , state , zip , longitude , latitude)
...
2.  What are the dimensions of the La Quinta's dataset? 
 ...
 the raw it's about the hotels address (more information about it's)
...
4.  Which states have the most and fewest Denny's locations? What about La Quinta? Is this surprising? Why or why not?=
 Dennys the most = CA    and the fewest = DE
La Quinta the most= TX   and the fewes = AG
For Denny's - Most
